...
// ── EDI tag map (ID Relation + datos EDI) ─────────────────────────────────
...
/**
 * Builds a map of tagid → EdiTagInfo by joining "ID Relation" and "datos EDI".
 * Used to classify single-country RFID tags as Departure or Arrival
 * when RFID data alone is insufficient (no cross-border reading).
 *
 * Join: "ID Relation".tagid → "datos EDI".ean (via s9id)
 */
export async function fetchEdiTagMap(): Promise<Map<string, EdiTagInfo>> {
  const headers = await getAuthHeaders();

  // Step 1: fetch all ID Relation rows (tagid → s9id)
  const idRelUrl = new URL(`${SUPABASE_URL}/rest/v1/${encodeURIComponent('ID Relation')}`);
  idRelUrl.searchParams.set('select', 'tagid,s9id');
  idRelUrl.searchParams.set('limit', '50000');

  const idRelRes = await fetch(idRelUrl.toString(), { headers });
  if (!idRelRes.ok) return new Map();
  const idRelRows: { tagid: string; s9id: string }[] = await idRelRes.json();

  // Build s9id → tagid lookup
  const s9idToTagId = new Map<string, string>();
  for (const row of idRelRows) {
    if (row.s9id && row.tagid) s9idToTagId.set(row.s9id, row.tagid);
  }

  // Step 2: fetch all datos EDI rows (ean/s9id → origin, destination)
