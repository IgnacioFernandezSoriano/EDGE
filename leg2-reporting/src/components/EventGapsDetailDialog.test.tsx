...
describe("EventGapsDetailDialog", () => {
  ...
  it("shows the weekday next to each date, and origin/dest gate+site", () => {
    render(<EventGapsDetailDialog open title="IN → JP" rows={rows} loading={false}
      onOpenChange={() => {}} onToggleExclude={() => {}} onSelectS9={() => {}} />);
    expect(screen.getAllByText(/\(\w{3}\)/).length).toBeGreaterThan(0);
    expect(screen.getByText("EV7")).toBeInTheDocument();
  ...
  it("calls onToggleExclude with the row and the new state", () => {
    ...
    render(<EventGapsDetailDialog open title="IN → JP" rows={rows} loading={false}
      onOpenChange={() => {}} onToggleExclude={onToggle} onSelectS9={() => {}} />);
    fireEvent.click(screen.getAllByRole("checkbox")[0]);
    expect(onToggle).toHaveBeenCalledWith(rows[0], true);
  });
  it("calls onSelectS9 with the s9code when the code button is clicked", () => {
    const onSelectS9 = vi.fn();
    render(<EventGapsDetailDialog open title="IN → JP" rows={rows} loading={false}
      onOpenChange={() => {}} onToggleExclude={() => {}} onSelectS9={onSelectS9} />);
