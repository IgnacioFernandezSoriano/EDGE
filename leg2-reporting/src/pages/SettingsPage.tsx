...
function ReprocessPanel({ deps = {} }: { deps?: SettingsDeps }) {
...
export default function SettingsPage({ deps = {} }: { deps?: SettingsDeps }) {
  return (
    <Tabs defaultValue="reprocess" className="mx-auto max-w-3xl p-6">
      <TabsList>
        <TabsTrigger value="reprocess">{strings.settings.tabReprocess}</TabsTrigger>
        <TabsTrigger value="comparisons">{strings.settings.tabComparisons}</TabsTrigger>
      </TabsList>
      <TabsContent value="reprocess">
        <ReprocessPanel deps={deps} />
      </TabsContent>
